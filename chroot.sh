...
echo -n "\e[31mEnter the root password:\e[0m "
passwd

##########################################################################

# Ask for a new username and create the user
echo -n "\e[31mEnter a username to create:\e[0m "
read username
useradd -m -g users -G wheel "$username"

# Set password for the new user
echo -n "\e[31mEnter the password for the user $username:\e[0m "
passwd "$username"
...
echo -n "\e[31mDo you have an Intel or Nvidia GPU? (intel/nvidia):\e[0m "
read gpu_choice
...
if [[ "$gpu_choice" == "intel" ]]; then
    ...
    echo "\e[31mIntel GPU drivers installed.\e[0m"

elif [[ "$gpu_choice" == "nvidia" ]]; then
    # Install Nvidia GPU drivers
    pacman -S nvidia nvidia-utils nvidia-lts --noconfirm
    echo "\e[31mNvidia GPU drivers installed.\e[0m"
else
    echo "\e[31mInvalid choice for GPU. Please enter 'intel' or 'nvidia'.\e[0m"
    exit 1
fi
...
echo "\e[31mModifying /etc/mkinitcpio.conf to include 'encrypt' and 'lvm2' in HOOKS...\e[0m"
...
echo "\e[31mRegenerating initramfs for linux and linux-lts kernels...\e[0m"
...
echo "\e[31mModifying /etc/locale.gen to enable 'en_US.UTF-8' and 'de_DE.UTF-8' locales...\e[0m"
...
echo "\e[31mUpdated /etc/locale.gen:\e[0m"
grep -E "en_US.UTF-8|de_DE.UTF-8" /etc/locale.gen

##########################################################################

# Generate the locales
echo "\e[31mRunning locale-gen...\e[0m"
locale-gen

##########################################################################

# Modify /etc/default/grub to include cryptdevice option
echo "\e[31mModifying /etc/default/grub to include 'cryptdevice=/dev/$partition3:volgroup0'...\e[0m"
...
# Verify the modification
echo "\e[31mUpdated GRUB_CMDLINE_LINUX_DEFAULT:\e[0m"
grep "^GRUB_CMDLINE_LINUX_DEFAULT=" /etc/default/grub

##########################################################################

# Create the EFI directory and mount the EFI partition
echo "\e[31mCreating /boot/EFI and mounting the EFI partition...\e[0m"
...
if ! grub-install --target=x86_64-efi --bootloader-id=grub_uefi --recheck; then
    echo "\e[31mGRUB installation failed. Exiting.\e[0m"
    exit 1
fi
...
echo "\e[31mCopying GRUB locale files...\e[0m"
...
echo "\e[31mRegenerating GRUB configuration file...\e[0m"
grub-mkconfig -o /boot/grub/grub.cfg

##########################################################################

# Enable necessary system services
echo "\e[31mEnabling GDM (GNOME Display Manager) and NetworkManager...\e[0m"
...
echo "\e[31mExiting chroot environment...\e[0m"
exit 0
